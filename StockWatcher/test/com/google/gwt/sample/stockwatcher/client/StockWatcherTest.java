	...
	Timer timer = new Timer() {		
		...
		public void run() {			
			...
			if (lastRefreshThrowable != null) {
				this.throwUncheckedException(lastRefreshThrowable);
			}
		...
		private void assertStockPrice(FlexTable stocksTable, int row, String price, String change) {
			assertEquals(price, stocksTable.getText(row, 1));
			assertEquals(change, stocksTable.getText(row, 2));
		}

		private void throwUncheckedException(Throwable lastRefreshThrowable) {
